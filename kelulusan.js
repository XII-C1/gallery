...
/*
  Ubah jumlah foto di sini.
  Misal foto kamu ada 40:
*/
const totalPhotos = 58;

/*
  Nama dasar file.
  Artinya JS akan mencari:
  kelulusan-1.jpg / kelulusan-1.png / kelulusan-1.webp / dst
*/
const photoFolder = "image/kelulusan";
const photoPrefix = "kelulusan";

/*
  Format yang akan dicoba otomatis.
  Jadi kamu tidak perlu ubah jpg/png satu-satu.
*/
...
function getPhotoClass(index){
  /*
    Ini pola layout otomatis.
    Kamu bebas ubah sesuai selera.
  */
...
function getPhotoBase(index){
  // Formatnya diubah jadi ngebaca spasi dan tanda kurung: kelulusan (1), dst.
...
function createPhotoCard(index){
  ...
  button.innerHTML = `
    <img 
      src="${firstSrc}" 
      alt="Foto kelulusan ${index}" 
      loading="lazy" 
      decoding="async"
    >
    <span>Foto Kelulusan ${index}</span>
  `;
...
// Fungsi mengganti foto dengan animasi geser
function showLightboxPhoto(direction){
  const photos = getVisiblePhotos();
  if(photos.length === 0) return;

  // 1. Kasih animasi keluar ke arah yang sesuai
  lightboxImg.classList.add(direction > 0 ? "slide-out-left" : "slide-out-right");

  // 2. Tunggu sebentar biar animasinya jalan, baru ganti fotonya
  setTimeout(() => {
    ...
    // 3. Pindahkan foto ke sisi berlawanan sebelum muncul
    ...
    // Allow reflow (biar browser mereset posisi sebelum transisi)
    void lightboxImg.offsetWidth;

    // 4. Munculkan foto ke tengah
    lightboxImg.classList.remove("slide-out-left", "slide-out-right");
  }, 250); // Waktu ini harus sama dengan transition di CSS (0.25s)
}

// ==========================================
// EVENT LISTENER BAWAAN (KLIK BUKA, TOMBOL X, KEYBOARD)
// ==========================================
...
if(closeLightbox){
  closeLightbox.addEventListener("click", closeLightboxNow);
}
...
if(lightbox){
  lightbox.addEventListener("click", event => {
    // Biar klik gambar nggak bikin lightbox ketutup, target harus ngepas di background hitamnya
...
// ==========================================
// FITUR 1: KLIK FOTO UNTUK GESER KANAN/KIRI
// ==========================================
lightboxImg.addEventListener("click", event => {
  event.stopPropagation(); // Biar lightbox gak ikut ketutup pas foto diklik
  
  const clickX = event.clientX;
  const screenWidth = window.innerWidth;

  // Kalau klik di setengah layar bagian kanan -> Next
  if(clickX > screenWidth / 2) {
    showLightboxPhoto(1);
  } else {
    // Kalau klik di setengah layar bagian kiri -> Prev
    showLightboxPhoto(-1);
  }
});

// ==========================================
// FITUR 2: SWIPE (GESER LAYAR) KHUSUS HP
// ==========================================
let touchStartX = 0;
let touchEndX = 0;
